...
def _build_architecture_document_legacy(path: Path) -> None:
    ...
    add_body(doc, "Runtime tickets, activity, article metadata, and verified resolutions use local SQLite. Reproducible sources are 27 synthetic ticket fixtures (including 16 verified fixes) and 15 approved Markdown articles spanning workplace IT, ecommerce, data pipelines, and Power BI/KPI support. Tickets are restricted operational data; pilot content requires named ownership, access, retention, and freshness controls.")
